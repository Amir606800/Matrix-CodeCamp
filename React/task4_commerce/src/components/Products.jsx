...
const Products = (props) => {
    ...
    const filteredProducts = props.productlar.filter(item=>
        item.title.toLowerCase().includes(props.inputValue.toLowerCase()) 
        && (item.price>=props.minimum && item.price<=props.maximum)  
    )
    
    const catFilteredProducts  = props.checkList.length > 0
    ? filteredProducts.filter(item => props.checkList.includes(item.category) )
    : filteredProducts;

    const lastFilteredProducts  = props.brandList.length > 0
    ? catFilteredProducts.filter(item => props.brandList.includes(item.brand))
    : catFilteredProducts;
    ...
    <>
        <div className="Products">
            <div className="head-product w-100 rounded-2 p-4 overflow-hidden d-flex flex-column justify-content-center align-items-start bg-dark-subtle">
                ...
                <p className="under-treasure">We have prepared special discounts for you on grocery products...</p>
                <button className="btn bg-white fw-bold rounded-5 ">Shop Now <FaLongArrowAltRight /></button>
            </div>
            <div className="middle-splitter d-flex flex-row align-items-center p-3 my-2 justify-content-between">
                <p>Showing {catFilteredProducts.length} of {props.productlar.length} results</p>
                <div className="right d-flex align-items-center gap-3 justify-content-center">
                    ...
                    <CiBoxList onClick={()=>{setDisplay("")}} className="icon" />
                    <FiGrid onClick={()=>{setDisplay("col-12 col-sm-6 col-md-4 col-xl-3 ")}} className="icon" />
                </div>
                
            </div>

            <div className="list justify-content-center  row">
                ...
                <div key={i} className={display}>
                    <div className={"item d-flex {} gap-2 card p-3 "}>
                        <div className="discount position-absolute rounded-start-1 rounded-end-4">
                            {item.discountPercentage >7 ? `${item.discountPercentage}%`: ``}
                            </div>
                        ...
                        <div className="price"><span className="text-danger fs-5">
                        ...
                        <div className="bott d-flex gap-5 align-items-center"><button className="bg-success text-white border-0 px-3 fs-4 py-1 rounded-3"><FiShoppingCart /></button><span className="text-success fw-bolder">IN STOCK</span></div>
                    </div>
                </div>
                ))}
